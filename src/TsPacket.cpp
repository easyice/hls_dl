...
bool CTsPacket::H264_Get_slice_type(BYTE &SliceType/*10:IDR */)
{
	BYTE slice_type = -1;
	...
	for(int i=start_pos;i<size;i++)
	{
		if((p_data[i] == 0) &&
			...
			(p_data[i+2] == 1))
		{
			nal_unit_type = (p_data[i+2+1])&0x1F;
			if (nal_unit_type == 5)
			{
				SliceType = 10;
				return true;
			...
			{
				slice_type = p_data[i+2+1+1];
				int tmp = te(slice_type);
				if (tmp != -1)
				{
					SliceType = tmp;
					return true;
...
bool	CTsPacket::Get_Sequenec_head_code()
{	
	int i;
	BYTE size;
	BYTE *p_data=m_pPacket;
